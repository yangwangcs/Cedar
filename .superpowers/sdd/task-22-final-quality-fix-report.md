...
- Deadline mutation probes: replacing either `SeedGraph` or `SeedBenchmarkScore`'s configured deadline with zero made the focused test fail with `ResourceExhausted: commit: append queue is full`; both callsites were restored.
