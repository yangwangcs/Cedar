...
Status: DONE (review follow-up implemented and focused verification complete).
...
- Builds now require an exact manifest/input segment set, validate CRC and decode every Task 7 page, enforce descriptor/region/header consistency, reject filename collisions, and serialize durable publication.
...
- `RetireBefore` persists a disabled generation marker so reopen cannot resurrect retired projections. Quarantine rejects current/pinned references and fsyncs its directory.
...
ctest -R 'ProjectionStore|QueryCanonical|KernelLifecycle': 30/30 PASS
git diff --check: PASS
```
