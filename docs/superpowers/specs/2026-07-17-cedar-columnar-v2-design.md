# Cedar Zone-Columnar SST v2 and Content-Addressed Blob Design
...
This document defines Cedar's second design stage: a clean-break Zone-Columnar SST v2 and Blob v2 storage layer. It evolves Cedar's original ideas rather than replacing them:
...
SST v2 and Blob v2 replace these paths. They do not wrap or extend them in place.
...
The first v2 format supports these physical types:
...
`Descriptor`, `EntryKind`, `ExternalRef`, `EdgeRef`, and `DESCRIPTOR64` are not part of the v2 protocol.
...
PageDirectory entries contain page type, ordinal, and 64-bit block-relative offset and length. A logical column may use several page fragments in one block; each fragment covers a continuous row range.
...
Values above the threshold enter BlobStore before transaction prepare. The transaction WAL and MemTable retain only a durable `BlobRefV2`.
...
BlockIndex entries are sorted by first logical key. Point and entity-range readers binary-search this index and follow continuation flags when a logical key or entity spans blocks.
...
Algorithms may move into v2 codecs only after direct tests. Old classes do not remain as hidden fallback implementations.

## 28. Relationship to Mainstream Designs

| System/design | Cedar v2 adopts | Cedar v2 keeps different |
|---|---|---|
...
4. granule/block builder and SST v2 format tests;
...
Columnar v2 is complete only when:

1. all durable data uses typed SST v2 pages or content-addressed Blob v2 records;
