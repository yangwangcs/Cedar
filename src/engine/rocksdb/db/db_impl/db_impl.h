...
class DBImpl : public DB {
 public:
  ...
  Status Flush(
      ...
      const std::vector<ColumnFamilyHandle*>& column_families) override;
  Status GetCedarMaintenanceSnapshot(CedarMaintenanceSnapshot* snapshot);
  Status RunCedarMaintenance(const CedarMaintenanceGrant& grant,
  ...
  int num_running_flushes() {
    mutex_.AssertHeld();
    return num_running_flushes_.load(std::memory_order_relaxed);
  }
 ...
 protected:
  ...
  Status WriteImpl(const WriteOptions& options, WriteBatch* updates,
                   ...
                   bool exclusive_write_group = false,
                   CedarEpochMetrics* cedar_epoch_metrics = nullptr,
                   std::atomic<bool>* wal_sync_critical = nullptr);

  Status WriteCedarEpoch(const CedarEpochOptions& options, WriteBatch* batch,
                         WalDurableCallback on_wal_durable,
                         void* callback_context,
                         CedarEpochMetrics* metrics);
  friend Status ROCKSDB_NAMESPACE::WriteCedarEpoch(
      DB* db, const CedarEpochOptions& options, WriteBatch* batch,
      WalDurableCallback on_wal_durable, void* callback_context,
      CedarEpochMetrics* metrics);
  Status WriteCedarEpochImpl(const WriteOptions& options, WriteBatch* batch,
                             WalDurableCallback on_wal_durable,
                             void* callback_context,
                             CedarEpochMetrics* metrics,
                             std::atomic<bool>* wal_sync_critical = nullptr);
 ...
 private:
  ...
  struct PrepickedCompaction {
    ...
    // run
    bool need_repick;
    bool cedar_sfm_space_reserved = false;
  };
  ...
  struct CedarGrantState {
    ...
    uint64_t sst_bytes_before = 0;
    bool installed = false;
    bool consumed = false;
    bool completed = false;
  };

  // Requires mutex_. Moves the selected native flush request to the front of
  // the queue and consumes at most one matching Cedar grant.
  bool PrepareCedarFlushSchedule(uint64_t* cedar_grant_id);
  bool PrepareCedarCompactionSchedule(uint64_t* cedar_grant_id,
                                      PrepickedCompaction** prepicked);
  void CompleteCedarFlushGrant(uint64_t cedar_grant_id, const Status& status,
                               uint64_t output_bytes);
  void CompleteCedarCompactionGrant(uint64_t cedar_grant_id,
                                    const Status& status,
                                    uint64_t output_bytes);
  ...
  void BackgroundCallFlush(Env::Priority thread_pri, uint64_t cedar_grant_id);
  ...
  InstrumentedCondVar cedar_maintenance_cv_;
  ...
  std::atomic<uint64_t> cedar_maybe_schedule_calls_{0};
  std::atomic<uint64_t> cedar_background_flush_calls_{0};
  std::atomic<uint64_t> cedar_manual_compaction_calls_{0};
  std::atomic<uint64_t> cedar_periodic_task_registrations_{0};
  std::atomic<uint64_t> cedar_recovery_flush_exceptions_{0};
  std::atomic<uint64_t> cedar_maintenance_generation_{0};
  // Protected by mutex_. The signature excludes sampling time so generation
  // advances only when Cedar-visible maintenance state changes.
  ...
  std::atomic<int> num_running_compactions_{0};
  ...
  std::atomic<int> num_running_flushes_{0};
