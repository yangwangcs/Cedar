...
Status DBImpl::RunManualCompaction(
  ...
  cedar_manual_compaction_calls_.fetch_add(1, std::memory_order_relaxed);
  ...
  autovector<uint64_t> memtable_ids_to_wait;
  {
    ...
    if (s.ok()) {
      if (cfd->imm()->NumNotFlushed() != 0 || !cfd->mem()->IsEmpty() ||
          ...
          IsRecoveryFlush(flush_reason)) {
        flush_reqs.emplace_back(
            FlushRequest{flush_reason, {{cfd, flush_memtable_id}}});
      ...
      if (immutable_db_options_.persist_stats_to_disk) {
        ...
        if (cfd_stats != nullptr && cfd_stats != cfd &&
          ...
          if (stats_cf_flush_needed) {
            ...
            flush_reqs.emplace_back(
                FlushRequest{flush_reason, {{cfd_stats, flush_memtable_id}}});
            memtable_ids_to_wait.emplace_back(
...
bool DBImpl::PrepareCedarFlushSchedule(uint64_t* cedar_grant_id) {
  mutex_.AssertHeld();
  assert(cedar_grant_id != nullptr);
  *cedar_grant_id = 0;

  auto recovery = std::find_if(
      flush_queue_.begin(), flush_queue_.end(), [](const FlushRequest& req) {
        return IsRecoveryFlush(req.flush_reason);
      });
  if (recovery != flush_queue_.end()) {
    if (recovery != flush_queue_.begin()) {
      FlushRequest request = std::move(*recovery);
      flush_queue_.erase(recovery);
      flush_queue_.push_front(std::move(request));
    }
    cedar_recovery_flush_exceptions_.fetch_add(1, std::memory_order_relaxed);
    return true;
  }
  ...
  auto request_input_bytes = [](const FlushRequest& request) {
    uint64_t bytes = 0;
    for (const auto& entry : request.cfd_to_max_mem_id_to_persist) {
      const uint64_t next = entry.first->imm()->ApproximateMemoryUsage();
      bytes = bytes > std::numeric_limits<uint64_t>::max() - next
                  ? std::numeric_limits<uint64_t>::max()
                  : bytes + next;
    }
    return bytes;
  };
  auto request_rank = [](const FlushRequest& request) {
    int rank = 3;
    for (const auto& entry : request.cfd_to_max_mem_id_to_persist) {
      const std::string& name = entry.first->GetName();
      if (name == "meta") {
        rank = std::min(rank, 0);
      } else if (name == kDefaultColumnFamilyName) {
        rank = std::min(rank, 1);
      } else if (name == "facts") {
        rank = std::min(rank, 2);
      }
  ...
  auto selected = flush_queue_.end();
  uint64_t selected_input_bytes = 0;
...
void DBImpl::CompleteCedarFlushGrant(uint64_t cedar_grant_id,
                                     const Status& status,
...
bool DBImpl::PrepareCedarCompactionSchedule(
    uint64_t* cedar_grant_id, PrepickedCompaction** prepicked) {
  mutex_.AssertHeld();
  assert(cedar_grant_id != nullptr);
  ...
  *prepicked = nullptr;
  ...
  auto selected = std::min_element(
      compaction_queue_.begin(), compaction_queue_.end(),
      [](const ColumnFamilyData* left, const ColumnFamilyData* right) {
        const auto rank = [](const ColumnFamilyData* cfd) {
          const std::string& name = cfd->GetName();
          if (name == "meta") return 0;
          if (name == kDefaultColumnFamilyName) return 1;
          if (name == "facts") return 2;
          return 3;
        };
        return rank(left) < rank(right);
      });
  if (selected == compaction_queue_.end()) {
    return false;
  }
  if (selected != compaction_queue_.begin()) {
    ColumnFamilyData* cfd = *selected;
    compaction_queue_.erase(selected);
    compaction_queue_.push_front(cfd);
  }

  LogBuffer log_buffer(InfoLogLevel::INFO_LEVEL,
                       immutable_db_options_.info_log.get());
  std::unique_ptr<TaskLimiterToken> task_token;
  ColumnFamilyData* cfd = PickCompactionFromQueue(&task_token, &log_buffer);
  if (cfd == nullptr) {
    return false;
  }
  ...
  JobContext picker_context(0, true);
  if (cfd->ioptions().compaction_style == kCompactionStyleUniversal &&
      cfd->user_comparator()->timestamp_size() == 0) {
    InitSnapshotContext(&picker_context);
  }
  std::unique_ptr<Compaction> compaction(cfd->PickCompaction(
      mutable_cf_options, mutable_db_options_, picker_context.snapshot_seqs,
      picker_context.snapshot_checker, &log_buffer));
  if (compaction == nullptr) {
  ...
  for (const auto& level : *compaction->inputs()) {
    for (const FileMetaData* file : level.files) {
      const uint64_t bytes = file->fd.GetFileSize();
      input_bytes = input_bytes > std::numeric_limits<uint64_t>::max() - bytes
                        ? std::numeric_limits<uint64_t>::max()
                        : input_bytes + bytes;
    }
  }
  ...
  bool sfm_space_reserved = false;
  if (!EnoughRoomForCompaction(cfd, *compaction->inputs(),
  ...
  auto* next = new PrepickedCompaction;
  next->compaction = compaction.release();
  next->manual_compaction_state = nullptr;
...
void DBImpl::CompleteCedarCompactionGrant(uint64_t cedar_grant_id,
                                          const Status& status,
                                          uint64_t output_bytes) {
  mutex_.AssertHeld();
...
void DBImpl::MaybeScheduleFlushOrCompaction() {
  mutex_.AssertHeld();
  cedar_maybe_schedule_calls_.fetch_add(1, std::memory_order_relaxed);
  const bool cedar_kernel_mode = immutable_db_options_.cedar_kernel_mode;
  ...
  while (!is_flush_pool_empty && unscheduled_flushes_ > 0 &&
    ...
    uint64_t cedar_grant_id = 0;
    if (cedar_kernel_mode && !PrepareCedarFlushSchedule(&cedar_grant_id)) {
      break;
    }
    TEST_SYNC_POINT_CALLBACK(
    ...
    fta->cedar_grant_id_ = cedar_grant_id;
  ...
  while (bg_compaction_scheduled_ + bg_bottom_compaction_scheduled_ <
    ...
    uint64_t cedar_grant_id = 0;
    PrepickedCompaction* cedar_prepicked = nullptr;
    if (cedar_kernel_mode &&
        !PrepareCedarCompactionSchedule(&cedar_grant_id, &cedar_prepicked)) {
      break;
    }
    CompactionArg* ca = new CompactionArg;
    ca->db = this;
    ca->compaction_pri_ = Env::Priority::LOW;
    ca->prepicked_compaction = cedar_prepicked;
    ca->cedar_grant_id_ = cedar_grant_id;
    bg_compaction_scheduled_++;
...
void DBImpl::BGWorkFlush(void* arg) {
  ...
  static_cast_with_check<DBImpl>(fta.db_)
      ->BackgroundCallFlush(fta.thread_pri_, fta.cedar_grant_id_);
...
void DBImpl::BGWorkCompaction(void* arg) {
  ...
  static_cast_with_check<DBImpl>(ca.db)->BackgroundCallCompaction(
      prepicked_compaction, Env::Priority::LOW, ca.cedar_grant_id_);
...
void DBImpl::BackgroundCallFlush(Env::Priority thread_pri,
                                 uint64_t cedar_grant_id) {
  cedar_background_flush_calls_.fetch_add(1, std::memory_order_relaxed);
  ...
  TEST_SYNC_POINT("DBImpl::BackgroundCallFlush:Start:2");
  {
    ...
    CompleteCedarFlushGrant(cedar_grant_id, s, output_bytes);
...
void DBImpl::BackgroundCallCompaction(PrepickedCompaction* prepicked_compaction,
                       ...
                       immutable_db_options_.info_log.get());
  {
    ...
    CompactionJobStats completed_stats;
    ...
    const uint64_t output_bytes =
        cedar_grant_id != 0 && s.ok() ? completed_stats.total_output_bytes : 0;
    CompleteCedarCompactionGrant(cedar_grant_id, s, output_bytes);
...
Status DBImpl::BackgroundCompaction(bool* made_progress,
  ...
  *made_progress = false;
  ...
  assert(completed_stats != nullptr);
  CompactionJobStats& compaction_job_stats = *completed_stats;
  ...
  bool sfm_reserved_compact_space =
      prepicked_compaction != nullptr &&
      prepicked_compaction->cedar_sfm_space_reserved;
  if (is_manual) {
