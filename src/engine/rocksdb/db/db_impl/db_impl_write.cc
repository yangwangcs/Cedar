...
class WalDurablePreReleaseCallback : public PreReleaseCallback {
 public:
  WalDurablePreReleaseCallback(WalDurableCallback callback, void* context)
      : callback_(callback), context_(context) {}

  Status Callback(SequenceNumber, bool, uint64_t, size_t,
                  size_t) override {
    callback_(context_);
    TEST_SYNC_POINT("DBImpl::WriteCedarEpoch:AfterWalDurableCallback");
    return Status::OK();
  }

 private:
  WalDurableCallback callback_;
  void* context_;
};

class ScopedWalSyncCritical {
 public:
  explicit ScopedWalSyncCritical(std::atomic<bool>* flag) : flag_(flag) {
    if (flag_ != nullptr) flag_->store(true, std::memory_order_release);
  }

  ~ScopedWalSyncCritical() {
    if (flag_ != nullptr) flag_->store(false, std::memory_order_release);
  }

  ScopedWalSyncCritical(const ScopedWalSyncCritical&) = delete;
  ScopedWalSyncCritical& operator=(const ScopedWalSyncCritical&) = delete;
...
Status DBImpl::WriteCedarEpochImpl(
    const WriteOptions& write_options, WriteBatch* batch,
    WalDurableCallback on_wal_durable, void* callback_context,
    CedarEpochMetrics* metrics, std::atomic<bool>* wal_sync_critical) {
  if (!write_options.sync) {
    return Status::InvalidArgument("WriteCedarEpoch requires sync=true");
  }
  if (write_options.disableWAL) {
    return Status::InvalidArgument(
        "WriteCedarEpoch requires WAL to be enabled");
  }
  if (batch == nullptr) {
    return Status::InvalidArgument("Batch is nullptr");
  }
  if (on_wal_durable == nullptr) {
    return Status::InvalidArgument("WAL durable callback is nullptr");
  }
  if (immutable_db_options_.enable_pipelined_write) {
    return Status::NotSupported(
        "WriteCedarEpoch does not support pipelined writes");
  }
  if (immutable_db_options_.unordered_write) {
    return Status::NotSupported(
        "WriteCedarEpoch does not support unordered writes");
  }
  ...
  if (status.ok()) {
    WalDurablePreReleaseCallback callback(on_wal_durable, callback_context);
    status = WriteImpl(
        write_options, batch, /*callback=*/nullptr, /*user_write_cb=*/nullptr,
        /*wal_used=*/nullptr, /*log_ref=*/0, /*disable_memtable=*/false,
        /*seq_used=*/nullptr, /*batch_cnt=*/0, &callback,
        /*post_memtable_callback=*/nullptr, /*wbwi=*/nullptr,
        /*exclusive_write_group=*/true, metrics, wal_sync_critical);
  }
  return status;
}

Status DBImpl::WriteCedarEpoch(const CedarEpochOptions& cedar_options,
                               WriteBatch* batch,
                               WalDurableCallback on_wal_durable,
                               void* callback_context,
                               CedarEpochMetrics* metrics) {
  if (!cedar_options.sync) {
    return Status::InvalidArgument("WriteCedarEpoch requires sync=true");
  }
  if (cedar_options.disable_wal) {
    return Status::InvalidArgument(
        "WriteCedarEpoch requires WAL to be enabled");
  }
  ...
  write_options.protection_bytes_per_key =
      cedar_options.protection_bytes_per_key;
  return WriteCedarEpochImpl(write_options, batch, on_wal_durable,
                             callback_context, metrics,
                             cedar_options.wal_sync_critical);
}
...
Status DBImpl::WriteImpl(const WriteOptions& write_options,
                         ...
                         std::shared_ptr<WriteBatchWithIndex> wbwi,
                         bool exclusive_write_group,
                         CedarEpochMetrics* cedar_epoch_metrics,
  ...
  WriteThread::Writer w(write_options, my_batch, callback, user_write_cb,
                        ...
                        exclusive_write_group, cedar_epoch_metrics);
  ...
  assert(!two_write_queues_ || !disable_memtable);
  {
    ...
    const uint64_t wal_before_preprocess = cur_wal_number_;
    status = PreprocessWrite(write_options, &wal_context, &write_context);
    if (cedar_epoch_metrics != nullptr && cur_wal_number_ != wal_before_preprocess) {
      ++cedar_epoch_metrics->wal_rotations;
    }
    if (!two_write_queues_) {
  ...
  if (exclusive_write_group) {
        TEST_SYNC_POINT_CALLBACK("DBImpl::WriteCedarEpoch:WriteGroup",
                             &write_group.size);
  }
  if (wbwi) {
  ...
  if (status.ok()) {
      ...
      if (status.ok() && !write_options.disableWAL) {
        ...
        io_s = WriteGroupToWAL(write_group, wal_context.writer, wal_used,
                               ...
                               *wal_context.wal_file_number_size,
                               cedar_epoch_metrics, wal_sync_critical);
      }
    } else {
      if (status.ok() && !write_options.disableWAL) {
        PERF_TIMER_GUARD(write_wal_time);
    ...
    if (wal_context.need_wal_sync) {
      ...
      if (status.ok() && synced_wals.IsWalAddition()) {
        ...
        const auto manifest_started_at = std::chrono::steady_clock::now();
        status = ApplyWALToManifest(read_options, write_options, &synced_wals);
        if (cedar_epoch_metrics != nullptr) {
          cedar_epoch_metrics->manifest_us += static_cast<uint64_t>(
              std::chrono::duration_cast<std::chrono::microseconds>(
                  std::chrono::steady_clock::now() - manifest_started_at)
                  .count());
        }
      }
    ...
    if (status.ok()) {
      if (exclusive_write_group) {
        TEST_SYNC_POINT("DBImpl::WriteCedarEpoch:BeforeMemTable");
      }
      PERF_TIMER_FOR_WAIT_GUARD(write_memtable_time);

      const auto memtable_started_at = std::chrono::steady_clock::now();
      if (!parallel) {
      ...
      if (cedar_epoch_metrics != nullptr) {
        cedar_epoch_metrics->memtable_insert_us += static_cast<uint64_t>(
            std::chrono::duration_cast<std::chrono::microseconds>(
                std::chrono::steady_clock::now() - memtable_started_at)
                .count());
      }
    }
  }
...
IOStatus DBImpl::WriteGroupToWAL(const WriteThread::WriteGroup& write_group,
  ...
  const auto wal_append_started_at = std::chrono::steady_clock::now();
  io_s = WriteToWAL(*merged_batch, write_options, log_writer, wal_used,
                    &log_size, wal_file_number_size, sequence);
  if (cedar_epoch_metrics != nullptr) {
    cedar_epoch_metrics->wal_append_us += static_cast<uint64_t>(
        std::chrono::duration_cast<std::chrono::microseconds>(
            std::chrono::steady_clock::now() - wal_append_started_at)
            .count());
  }
  ...
  if (io_s.ok() && need_wal_sync) {
    ScopedWalSyncCritical sync_critical(wal_sync_critical);
    const auto wal_sync_started_at = std::chrono::steady_clock::now();
    ...
    if (cedar_epoch_metrics != nullptr) {
      cedar_epoch_metrics->wal_sync_us += static_cast<uint64_t>(
          std::chrono::duration_cast<std::chrono::microseconds>(
              std::chrono::steady_clock::now() - wal_sync_started_at)
              .count());
    }
  }
