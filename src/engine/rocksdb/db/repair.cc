...
bool IsCedarParquetTableFile(const std::shared_ptr<FileSystem>& fs,
  ...
  Slice header;
  return file
             ->Read(0, header_bytes.size(), file_options.io_options, &header,
                    header_bytes.data(), nullptr)
             .ok() &&
         header == Slice("PAR1");
}
...
class Repairer {
 public:
  Repairer(const std::string& dbname, const DBOptions& db_options,
      ...
      : dbname_(dbname),
        ...
        cedar_parquet_mopts_(MutableCFOptions(default_cf_opts_)),
        ...
        db_lock_(nullptr),
        closed_(false) {
    cedar_parquet_mopts_.table_factory = std::shared_ptr<TableFactory>(
        NewCedarParquetFactTableFactory(cedar_parquet::CedarParquetTableOptions()));
 ...
 private:
  ...
  MutableCFOptions cedar_parquet_mopts_;
  ...
  Status ScanTable(TableInfo* t) {
    ...
    t->meta.fd = FileDescriptor(t->meta.fd.GetNumber(), t->meta.fd.GetPathId(),
                                file_size);
    const MutableCFOptions& table_mopts =
        status.ok() && IsCedarParquetTableFile(env_->GetFileSystem(), fname,
                                                file_options_)
            ? cedar_parquet_mopts_
            : default_mopts_;
    ...
    if (status.ok()) {
      ...
      status = table_cache_->GetTableProperties(
          file_options_, read_options, icmp_, t->meta, &props, table_mopts);
    }
    if (status.ok()) {
      auto s =
    ...
    if (status.ok()) {
      ...
      InternalIterator* iter = table_cache_->NewIterator(
          ...
          nullptr /* range_del_agg */, table_mopts,
