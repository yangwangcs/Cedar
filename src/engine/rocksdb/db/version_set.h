...
class VersionStorageInfo {
 public:
  ...
  uint64_t GetAverageValueSize() const {
    const uint64_t accumulated_raw_size =
        accumulated_raw_key_size_ + accumulated_raw_value_size_;
    // Table formats that do not persist raw key/value statistics cannot
    // supply a meaningful delete-compensation estimate during recovery.
    if (accumulated_num_non_deletions_ == 0 || accumulated_raw_size == 0 ||
        accumulated_file_size_ == 0) {
      return 0;
    }
    return accumulated_raw_value_size_ / accumulated_num_non_deletions_ *
           accumulated_file_size_ / accumulated_raw_size;
  }
