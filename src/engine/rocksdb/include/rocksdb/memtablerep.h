...
class PartitionedVersionRadixFactory : public MemTableRepFactory {
 public:
  ...
  explicit PartitionedVersionRadixFactory(Options options)
      : options_(std::move(options)) {}

  static const char* kClassName() { return "PartitionedVersionRadixFactory"; }
  static const char* kNickName() { return "partitioned_version_radix_v1"; }
  const char* Name() const override { return kClassName(); }
  const char* NickName() const override { return kNickName(); }
