...
struct DBOptions {
  ...
  // Cedar Kernel Mode reserves maintenance admission for Cedar. RocksDB keeps
  // ownership of every maintenance operation and starts one only after a
  // Cedar-native grant is installed.
  bool cedar_kernel_mode = false;

  // RocksDB does not register periodic statistics, info-log, or triggered
  // compaction tasks. Cedar may still leave ordinary flush/compaction
  // scheduling enabled through cedar_kernel_mode=false.
  bool cedar_disable_periodic_tasks = false;
