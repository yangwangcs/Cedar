...
// BackupEngine normally opens table files with SstFileDumper to obtain the DB
// session identity used in shared-file names. Cedar facts tables are Parquet
// files with a PAR1 envelope, so their footer is intentionally not an SST
// footer. Recognize that envelope before allowing the backup path to fall back
// to its full-file checksum naming scheme.
bool IsParquetTableFile(const std::shared_ptr<FileSystem>& fs,
  ...
  if (!fs->GetFileSize(path, file_options.io_options, &file_size, nullptr).ok() ||
      file_size < 8) {
    return false;
  }
  std::unique_ptr<FSRandomAccessFile> file;
  if (!fs->NewRandomAccessFile(path, file_options, &file, nullptr).ok()) {
    return false;
  }
  std::array<char, 4> start{};
  std::array<char, 4> end{};
  Slice read;
  if (!file->Read(0, start.size(), file_options.io_options, &read,
                  start.data(), nullptr)
           .ok() ||
      read != Slice("PAR1")) {
    return false;
  ...
  return file
             ->Read(file_size - end.size(), end.size(), file_options.io_options,
                    &read, end.data(), nullptr)
             .ok() &&
         read == Slice("PAR1");
}
...
IOStatus BackupEngineImpl::AddBackupFileWorkItem(
  ...
  if (shared && shared_checksum) {
    if (GetNamingNoFlags() != BackupEngineOptions::kLegacyCrc32cAndFileSize &&
      ...
      if (s.IsNotFound() ||
          (s.IsCorruption() && IsParquetTableFile(
                                   db_fs_, src_path, src_env_options,
                                   src_temperature))) {
        // db_id and db_session_id will be empty, which is OK for old files
        // and for Cedar Parquet tables. The latter use the checksum-and-size
        // backup naming path because their table format has no RocksDB SST
        // properties footer.
      } else if (!s.ok()) {
