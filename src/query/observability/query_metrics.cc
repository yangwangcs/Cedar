...
StatusOr<std::string> EncodeQueryStatistics(const QueryStatisticsSnapshot& snapshot) {
  ...
  Put64(&out, snapshot.generation_id); Put64(&out, snapshot.base_seq.value); Put32(&out, static_cast<uint32_t>(snapshot.columns.size()));
  for (const auto& c : snapshot.columns) {
...
StatusOr<QueryStatisticsSnapshot> DecodeQueryStatistics(const std::string& in) {
  ...
  snapshot.checksum=expected; if(p!=payload_end)return Status::Corruption("query statistics", "trailing payload"); return snapshot;
}
